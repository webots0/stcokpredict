...
# 创建表格 在test1 数据库里面创建名字为test的表格 有日期和股票代码
...
# 添加数据 在test1数据库里面的test表格添加一列
#cursor.execute("ALTER TABLE test ADD COLUMN 收盘价 DECIMAL(10, 3) NOT NULL;")

# 修改数据 在test1数据库里面创建名字为test的表格修改其中一列
...
# 查寻数据
cursor.execute("SELECT 股票代码 FROM test;")
